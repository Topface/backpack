(function(module) {
                ...
                self.redis.hgetall("files:sizes", function(error, sizes) {
                    ...
                    if (files) {
                        Object.keys(files).forEach(function(id) {
                            ...
                            self.sizes[id] = (sizes && sizes[id]) ? +sizes[id].toString() : 0;
                        });
                    }
    ...
    FileManager.prototype.nodeKeySerializer = function(key) {
        return key;
    };

    FileManager.prototype.nodeKeyUnserializer = function(serialized) {
        return serialized;
    };
    ...
    FileManager.prototype.nodeInfoSerializer = function(file, offset, length) {
        return file + ":" + offset + ":" + length;
    };

    FileManager.prototype.nodeInfoUnserializer = function(value) {
        ...
        self.retryAfterLoad(function() {
            ...
            self.files[sorted[0]].file.getWriter(name, length, function(error, writer) {
                ...
                writer.on("close", function() {
                    var id    = sorted[0],
                        value = self.nodeInfoSerializer(id, writer.getOffset(), length);

                    self.redis.set(self.nodeKeySerializer(name), value, function(error) {
                        if (error) {
                            console.log(error);
                        }
                    });
    ...
    FileManager.prototype.getFileReader = function(name, callback) {
        ...
        self.retryAfterLoad(function() {
            self.redis.get(self.nodeKeySerializer(name), function(error, info) {
                if (error) {
                    return callback(error);
                }
                ...
                info = self.nodeInfoUnserializer(info);
